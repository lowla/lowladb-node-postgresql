(function (exports) {
  ...
  function removeDocument (lowlaId, versionPreDelete) {
    ...
    return new Promise(function (resolve, reject) {
      try{
        ...
        datastore._getTableDefinition(lowlaId.collectionName).then(
          function (tableDef) {
            var query = lowlaId.id;
            if (versionPreDelete) {
              query._version = versionPreDelete;
              if(!tableDef.hasColumn('_version')){
                tableDef.addColumn('_version');
              }
            }
            var sqlBuilder = tableDef.delete().where(query).returning(tableDef.star());
            datastore.config.db.query(sqlBuilder.toQuery(), function(err, result) {
              if (err) {
                return reject(err);
              } else if( 1 > result.rows.length ) {
        ...
        promises.push(
          datastore._getTableDefinition(tableName).then(
            function (tableDef) {
              ...
              var primaryKeys =[];
              for (var col in tableDef.columns){
  ...
  function _findInTable (tableName, query) {
    ...
    return datastore._getTableDefinition(tableName).then(
      function (tableDef) {
        ...
        if(query){
          var queryOps=[];
          for (var prop in query){ //convert special ops (e.g. $gte) into 'sql' function calls
            var queryVal = query[prop];
            var op='equals';
            var val = queryVal;
            if(_.isObject(queryVal)){
              for (var p in queryVal){
                if (p.indexOf('$')===0){
                  op = p.substring(1);
                  val = queryVal[p];
                }
  ...
  function _updateOpToDocument(tableDef, ops, isNew){
    ...
    for (var incrementField in incrementOps){
